...
//	}
	...
	public ResponseEntity<DeliveryPersonDto> getPersonById(@PathVariable int id){
		DeliveryPersonDto d=deliveryPersonService.getDeliveryPersonDtoById(id);
		return new ResponseEntity<DeliveryPersonDto>(d,HttpStatus.ACCEPTED);
	}
	
	@PutMapping("/deliveryperson/{id}/{status}")
	public ResponseEntity<Boolean> changeStatus(@PathVariable int id,@PathVariable Boolean status){
		boolean val= deliveryPersonService.changeStatus(id,status);
