...
public class DaoToEntityConverter {
	...
	public static Admin adminSignIn(AdminDto adminDto) {
		Admin ad = new Admin();
		BeanUtils.copyProperties(adminDto, ad);
		return ad;
	}
	public static RestaurantDto restaurantEntityToRestaurantDto(Restaurant rest) {
		RestaurantDto restHomePageDto = new RestaurantDto();
...
//		BeanUtils.copyProperties(foodItem, foodItemHomePageDto, "orderItem");
   ...
   public static OrdersDto orderToOrderDto(Orders order) {
	   ...
	   orderDto.setRestaurant(restaurantEntityToRestaurantDto(order.getRestaurantId()));
